...
export const NGOList =({ onFormSwitch }) => {
  ...
  const getNGOs = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:4000/api/Admin/ngos', {
        headers: {
  ...
  const fetchData = async () => {
    ...
    const filteredData = ngos.filter(req => req.username.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="admincontainer1L">
        <div className="buttons-containerL">
          ...
          <Link to="/AdminDashboard"><button className="b1-admin">
            ...
            <Link to="/DonatorList"><button className="b1">
              <h1 className="textngoL">Donators List</h1>
            </button></Link>
          </div>
          </div>
